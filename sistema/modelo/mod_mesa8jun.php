<?php
	...
	class mesas extends conexion{
		...
		function consultar_cant_mesas(){
			$sql="SELECT pk_estado, descripcion_estado, pk_municipio, descripcion_municipio, (SELECT COUNT(pk_mesa) FROM sistema.mesas WHERE sistema.mesas.fk_municipio=ubicacion.municipio.pk_municipio) AS cant_mesas  FROM ubicacion.municipio INNER JOIN ubicacion.estado ON ubicacion.municipio.fk_estado=ubicacion.estado.pk_estado WHERE (SELECT COUNT(pk_mesa) FROM sistema.mesas WHERE sistema.mesas.fk_municipio=ubicacion.municipio.pk_municipio)>0 ORDER BY pk_municipio";
			$query=pg_query($this->pg_connect, $sql) or die("Error al consultar". pg_last_error());
			return $query;
		}
