...
<head>
	...
	<script>
			...
			function download() {
				...
				imageLink.download = 'integrantes_por_estado.png';
			...
			const data = {
		        labels:[
		            <?php
					    ...
					    $obj_mesa=new mesas();
					    $condicion="";				    
					    $consultar_cant_integrantes=$obj_mesa->consultar_cant_integrantes_e($condicion);
					    while ($resultado=pg_fetch_array($consultar_cant_integrantes)) {
					?>
			             '<?php echo ucwords(strtolower(preg_replace('[\n|\r|\n\r]', '',$resultado["descripcion_estado"])));?>:'+' <?php echo number_format($resultado["cant_integrantes"], 0, ",", ".");?>',
		            <?php		                					                    
		            	}
		            ?>			            	
		        ],
